...
myApp.controller('dyymCtl', function ($stateParams, $scope, $http, $timeout, $rootScope, factory_getValue) {
    $scope.showPrompt = 0;  //1是内容不足10条 2是内容超过30
    var $slider = $('#slider');
    var sliderType = 1;
    var maxCount;
    var dqIndex = 0;
    ...
    $scope.loadData = function () {
        ...
        $scope.viewAlbumGAItem = function (type, event,d,storeIndex) {
            ...
            callNative('viewAlbumGAItem', 1, JSON.stringify({type: type,studentRecordId: $rootScope.globalData.studentRecordId, position: position, theme:$rootScope.themesKey,data:JSON.stringify(data)}));
        }
        ...
        $scope.addContent=function () {
            var count=$rootScope.allRecord.me.length+$rootScope.allRecord.yuan.length+$rootScope.introduces.length;
            console.log(count)
            if(count>30){
                callNative('showMessage', 1, count);
            }else {
                callNative('viewAlbumGAItem', 1, JSON.stringify({type: 6,studentRecordId: $rootScope.globalData.studentRecordId, position: -1, theme:$rootScope.themesKey}));
            }
        }
    ...
    function init(data) {
        ...
        //园长记录12
        $scope.modularDataStories = [];
        if (data.stories && data.stories.length > 0) {
            $scope.modularDataStories = transformationObj(data, 'stories');
        }
        if (data.inviteStories && data.inviteStories.length > 0) {
            $scope.modularDataStories = $scope.modularDataStories.concat(transformationObj(data, 'inviteStories'))
        }
        $scope.modularDataStories = $scope.modularDataStories.sort(function (a, b) {
            return b.createAt - a.createAt
        })

        //个人记录14
        $scope.modularDataJiaStories = [];
        if (data.jiaStories && data.jiaStories.length > 0) {
            $scope.modularDataJiaStories = transformationObj(data, 'jiaStories');
        }
        $scope.modularDataJiaStories = $scope.modularDataJiaStories.sort(function (a, b) {
            return b.createAt - a.createAt
        });

        $rootScope.allRecord = {
            me: $scope.modularDataJiaStories,
            yuan: $scope.modularDataStories
        }


        //判断处理
        $scope.innerPageCount=1;
        $rootScope.introduces=['meIndex'];
        var gardenCount = 0,
            meCount = $scope.modularDataJiaStories.length;
        if ($scope.data.schoolPhoto) {//园所首页
            gardenCount++;
            $rootScope.introduces.push('schoolIndex');
        }
        if ($scope.data.photo) {//全家福
            gardenCount++;
            $rootScope.introduces.push('classIndex');
        }
        if ($scope.data.labels && $scope.data.labels.length > 0) {//期末寄语默认
            gardenCount++;
            $rootScope.introduces.push('ydIndex');
        }

        var allGardenCount = gardenCount + $scope.modularDataStories.length;
        var allMeCount = meCount + 1;

        if (allGardenCount + allMeCount <= 30) {
            $scope.recordArr = $scope.recordArr.concat($scope.modularDataStories, $scope.modularDataJiaStories);
        } else {
            if (allGardenCount < 15 && allMeCount > 15) {
                $scope.recordArr = $scope.recordArr.concat($scope.modularDataStories, $scope.modularDataJiaStories.slice(0, 30 - allGardenCount-1));
                if((allGardenCount+allMeCount-1)%2==0){
                    $scope.innerPageCount=2;
                }
            }
            else if (allGardenCount > 15 && allMeCount < 15) {
                $scope.recordArr = $scope.recordArr.concat($scope.modularDataStories.slice(0, 30 - allMeCount), $scope.modularDataJiaStories);
            } else {
                $scope.innerPageCount=2;
                $scope.recordArr = $scope.recordArr.concat($scope.modularDataStories.slice(0, 15 - gardenCount), $scope.modularDataJiaStories.slice(0, 14));
            }
        }
        ...
        //判断内页
        if((allGardenCount+allMeCount)%2==0){
            $scope.innerPageCount=2;
        }

        $scope.resetPrompt(allGardenCount + allMeCount);


        console.log(allGardenCount,allMeCount);
        console.log($scope.recordArr);
        console.log($scope.allRecord);
    }

    $scope.resetPrompt = function (count) {
        if(count<6){
            $scope.showPrompt=1;
        }
        if(count>30){
