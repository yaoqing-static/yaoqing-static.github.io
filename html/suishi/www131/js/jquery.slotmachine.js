...
;(function($) {
	...
	*/
	$.fn.slotMachine = function(settings){
		...
		*/
		function _stop( nowOrRepeations, getElementFn ){
			...
			if( typeof getElementFn==="function" ){
				...
				if( settings.repeat ){
					rnd = _getNext();
				}else if(settings.index !=undefined && settings.index>=0){
					settings.index = settings.index % $titles.size();
					rnd = {
						index : settings.index,
						el : $titles.get(settings.index)
					};
				}
				else{
					rnd = _getRandom();
		...
		*/
		...
		/**
		  ...
		  * @param int count - Number of shuffles (undefined to make infinite animation
		*/
		$slot.shuffle = function(endIndex,  oncomplete ,count){
	...
	$.fn.slotMachines = function (settings){
		var defaults = {
			active	: 0,
			delay	: 200
		};
		...
		var isRunning = false;
		var _active;
		function _shuffleTemp(slot, active){
			_active.push(active);
			tempIndex++;
			if(tempIndex >= slotArr.length){
		...
		$(this).each(function (index){
			var temp = {};
			temp = $.extend(temp, settings);
			temp.delay = temp.delay + 50*index;//temp.delay * (0.8 + 0.4*Math.random());
			slotArr.push($(this).slotMachine(temp));
		});
		$slot.size = function (){
		...
		$slot.shuffle = function(endIndex,  oncomplete ,count){
			...
			for(var i = 0; i < slotArr.length; i++){
				//var rnd = Math.random();
				temp = i*2;//rnd <=0.33?0:(rnd<=0.66?2:4);
				slotArr[i].shuffle(endIndex[i], _shuffleTemp, count+temp);
			}
		};
		$slot.isRunning = function (){
			return isRunning;
